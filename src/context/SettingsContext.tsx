...
import { DEFAULT_PATCH } from '@/data/units';
...
interface SettingsContextType {
  ...
  patchVersion: string;
  setPatchVersion: (val: string) => void;
}
...
export const SettingsProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const [patchVersion, setPatchVersionState] = useState<string>(DEFAULT_PATCH);
  ...
  useEffect(() => {
    ...
    const storedPatch = localStorage.getItem('sc2-atlas-patch-version');
    if (storedPatch) {
      setPatchVersionState(storedPatch);
    }
  ...
  const setPatchVersion = (val: string) => {
    setPatchVersionState(val);
    localStorage.setItem('sc2-atlas-patch-version', val);
  };
  ...
  return (
    <SettingsContext.Provider
      value={{
        ...
        setAnalyticsEnabled,
        patchVersion: mounted ? patchVersion : DEFAULT_PATCH,
        setPatchVersion,
      }}
    >
      {children}
