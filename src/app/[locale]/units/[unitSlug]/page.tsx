...
import { ChevronLeft, Info, Zap, ArrowUpCircle, Compass, CheckCircle, AlertTriangle, Swords, Lightbulb } from 'lucide-react';
...
export default function UnitDetailPage() {
  ...
  const tacticalRole = unit ? (t.has(`units.${unit.id}.tacticalRole`)
    ? t(`units.${unit.id}.tacticalRole`)
    : unit.tacticalRole || t(`units.${unit.id}.shortDescription`) || '') : '';

  const strengths = unit ? (t.has(`units.${unit.id}.strengths`)
    ? (t.raw(`units.${unit.id}.strengths`) as string[])
    : unit.strengths || []) : [];

  const weaknesses = unit ? (t.has(`units.${unit.id}.weaknesses`)
    ? (t.raw(`units.${unit.id}.weaknesses`) as string[])
    : unit.weaknesses || []) : [];

  const goodAgainst = unit ? (t.has(`units.${unit.id}.goodAgainst`)
    ? (t.raw(`units.${unit.id}.goodAgainst`) as string[])
    : unit.goodAgainst || []) : [];

  const vulnerableAgainst = unit ? (t.has(`units.${unit.id}.vulnerableAgainst`)
    ? (t.raw(`units.${unit.id}.vulnerableAgainst`) as string[])
    : unit.vulnerableAgainst || []) : [];

  const usageTip = unit ? (t.has(`units.${unit.id}.usageTip`)
    ? t(`units.${unit.id}.usageTip`)
    : unit.usageTip || '') : '';

  const gamePhase = unit ? (t.has(`units.${unit.id}.gamePhase`)
    ? (t.raw(`units.${unit.id}.gamePhase`) as string[])
    : unit.gamePhase || []) : [];

  useEffect(() => {
    if (unit) {
      trackEvent('view_unit_details', 'engagement', unit.slug, undefined, { name: unitName, race: unit.race });
    }
  }, [unit, unitName]);
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <p className="text-gray-500 mb-8">The requested unit profile does not exist in our tactical database.</p>
          <Link
            href="/"
            onClick={() => trackEvent('nav_back_clicked', 'navigation', 'command_center', undefined, { from: '404_unit', unitSlug })}
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="mb-12">
          <Link 
            ...
            onClick={() => trackEvent('nav_back_clicked', 'navigation', 'race_roster', undefined, { from: 'unit_detail', race: unit.race, unit: unit.slug })}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <GlassPanel className="p-8 border-white/5 space-y-6">
              <div className="space-y-2">
                <h3 className="text-sm font-bold text-gray-500 uppercase tracking-widest flex items-center gap-2">
                  <Compass className="w-4 h-4" style={{ color: race.theme.primary }} /> {tUi('tacticalOverview')}
                </h3>
                {tacticalRole && (
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-4">
                  {unit.abilities.map(ability => (
                    <GlassPanel key={ability.id} className="p-6 border-white/5 flex gap-6 items-center">
              ...
              <div className="space-y-4 mt-8">
                ...
                <div className="grid grid-cols-1 gap-4">
                  {unit.upgrades.map(upgrade => (
                    <GlassPanel key={upgrade.id} className="p-6 border-white/5 flex gap-6 items-center">
          ...
          <div className="lg:col-span-5 space-y-8">
            ...
            <GlassPanel className="p-6 border-white/5 bg-blue-900/10 border-blue-500/10">
              <h3 className="text-sm font-bold text-blue-500 uppercase tracking-widest mb-4">{t('ui.productionIntelligence')}</h3>
              <div className="space-y-4">
                <div className="flex justify-between items-center text-sm">
                  <span className="text-gray-500">{t('ui.facility')}:</span>
                  <span className="text-white font-bold">
                    {unit.production.producedFrom ? t(`tech.${unit.production.producedFrom}`) : 'N/A'}
                  </span>
                </div>
                <div className="flex justify-between items-start text-sm">
                  ...
                  <div className="text-right">
                    {unit.production.requirements?.map(req => (
                      <div key={req} className="text-white font-bold">{t(`tech.${req}`)}</div>
