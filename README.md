...
## 🎨 Formato y Calidad de Código (Linting)

Este proyecto cuenta con reglas de formato y linting unificadas para asegurar la consistencia del código.

### Herramientas y Configuración

- **EditorConfig**: Define las reglas de formato básicas del editor (indentación por espacios de 2 columnas, finales de línea LF, añadir línea en blanco al final de los archivos, etc.).
- **Prettier**: Se encarga del formateo consistente del código (comillas simples en TypeScript/CSS, doble comilla en JSX, print width de 100 caracteres, etc.).
- **ESLint**: Se utiliza para el análisis de calidad de código y detección de errores (con Flat Config en `eslint.config.mjs`). Se ha integrado con Prettier para evitar conflictos de formateo.

### Comandos de Terminal

Puedes comprobar y aplicar las reglas desde la terminal mediante los siguientes scripts de npm:
...
- **Corregir errores de linter automáticamente**:
  ...
  npm run lint:fix
  ```
- **Comprobar todo (linter + formato)**:
  ```bash
  npm run check-all
  ```
- **Corregir todo automáticamente (linter + formato)**:
  ```bash
  npm run fix-all
  ```
